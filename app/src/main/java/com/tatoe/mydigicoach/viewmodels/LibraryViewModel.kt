...
class LibraryViewModel(var application: Application, var db: FirebaseFirestore) : ViewModel() {
        ...
        docRef.get().addOnSuccessListener { docs ->
            if (docs.isEmpty) {
                ...
                if (Utils.isConnectedToInternet(application)) {
                    Timber.d("Weird, query is wrong? or db has been cleared")
                } else {
                    string =
                        "There is no Internet connection nor local copy, connect to Internet to browse the Store exercises"
                }
                infoText.postValue(string)
                ...
                for (document in docs) {
                    ...
                    var docRef2 =
                        db.collection("store_exercises").document(document.id)
                            .collection("exercises")
                    docRef2.get().addOnSuccessListener { docs2 ->
                        for (doc in docs2) {
                            exercisePairList.add(
                                MyCustomStoreExercise(
                                    doc.toObject(MyCustomFirestoreTransferExercise::class.java).toExercise(),
                                    exerciseCategory
                                )
                            )
                        }
    ...
    fun importExercises(toImportStoreExercises: java.util.ArrayList<MyCustomStoreExercise>) =
...
//    }
    ...
    fun addBunchOfStubStoreExercises() {
        ...
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Snatch"),
            1 to hashMapOf("Description" to "Lift bar from ground, hit at hips and gain control over bar either as power snatch or normal snatch (full depth)")
        )
        var exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        olympicLiftingExes.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Hang snatch"),
            1 to hashMapOf("Description" to "Lift bar from hips, gain control over bar either as power snatch or normal snatch (full depth)")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        olympicLiftingExes.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Clean"),
            1 to hashMapOf("Description" to "Lift bar from ground, hit at hips and gain control over bar either as power clean or normal clean (full depth)")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        olympicLiftingExes.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Hang Clean"),
            1 to hashMapOf("Description" to "Lift bar from hips, clean")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        olympicLiftingExes.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Clean & jerk"),
            1 to hashMapOf("Description" to "Lift bar from ground, hit at hips and gain control over bar shoulders")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        olympicLiftingExes.add(exe)
        ...
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "3km"),
            1 to hashMapOf("Description" to "3km optimal time 11:00")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        longDistRuns.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "5km"),
            1 to hashMapOf("Description" to "5km optimal time 20:00")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        longDistRuns.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "10km"),
            1 to hashMapOf("Description" to "10km optimal time 45:00")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        longDistRuns.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Half marathon"),
            1 to hashMapOf("Description" to "21.098km")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        longDistRuns.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Marathon"),
            1 to hashMapOf("Description" to "42.196km lol have fun")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        longDistRuns.add(exe)
...
//        longDistRuns.add(Exercise("Marathon","42.196km lol have fun"))
        ...
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "30m"),
            1 to hashMapOf("Description" to "between 5 and 8 reps, recovery of 3/4 minutes minimum")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        sprints.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "100m"),
            1 to hashMapOf("Description" to "2 sets of 10 runs at 85% intensity, with 1-2 min recovery, full rest between sets")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        sprints.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Lactic pyramid"),
            1 to hashMapOf("Description" to "100-120-150-180-150-120-100 recovery of 5-6 min")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        sprints.add(exe)
...
//        sprints.add(Exercise("Lactic pyramid","100-120-150-180-150-120-100 recovery of 5-6 min"))
        ...
        val powerlifting = arrayListOf<Exercise>()
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Squats"),
            1 to hashMapOf("Description" to "Full depth squats")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        powerlifting.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Bench Press"),
            1 to hashMapOf("Description" to "Touch chest for max gains")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        powerlifting.add(exe)
        exeHashMap = hashMapOf(
            0 to hashMapOf("Name" to "Deadlift"),
            1 to hashMapOf("Description" to "Technique-wise make sure straight back, and engaging posterior chain when starting lift, start with legs not with back")
        )
        exe = Exercise(exeHashMap)
        exe.md5 = MD5Encrypter.getMD5(exe)
        powerlifting.add(exe)
...
//        powerlifting.add(Exercise("Deadlift","Technique-wise make sure straight back, and engaging posterior chain when starting lift, start with legs not with back"))
        ...
        db.collection("store_exercises").document("olympic_lifting")
            .set(hashMapOf("name" to "Olympic Lifting"))
        db.collection("store_exercises").document("long_distance_running")
            .set(hashMapOf("name" to "Long distance"))
        db.collection("store_exercises").document("sprinting").set(hashMapOf("name" to "Sprinting"))
        db.collection("store_exercises").document("powerlifting")
            .set(hashMapOf("name" to "Powerlifting"))


        for (exercise in olympicLiftingExes) {
            db.collection("store_exercises").document("olympic_lifting").collection("exercises")
                .document(exercise.name)
                .set(MyCustomFirestoreTransferExercise(exercise))
                ...
                .addOnFailureListener { e ->
                    Timber.d("Error writing document: $e")
                }
        }

        for (exercise in longDistRuns) {
            db.collection("store_exercises").document("long_distance_running")
                .collection("exercises").document(exercise.name)
                .set(MyCustomFirestoreTransferExercise(exercise))
                .addOnSuccessListener { Timber.d("DocumentSnapshot successfully written!") }
                .addOnFailureListener { e -> Timber.d("Error writing document: $e") }
        }
        ...
        for (exercise in sprints) {
            db.collection("store_exercises").document("sprinting").collection("exercises")
                .document(exercise.name)
                .set(MyCustomFirestoreTransferExercise(exercise))
                .addOnSuccessListener { Timber.d("DocumentSnapshot successfully written!") }
                .addOnFailureListener { e -> Timber.d("Error writing document: $e") }
